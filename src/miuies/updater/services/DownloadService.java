	...
	public void onStart(Intent intent, int startid){
        ...
        while((j < mirrors.length)&&(!found)){
			try {
				...
				Log.d("MIUIEsUpdater", "El archivo "+mirrors[j]+archivo+" no se encontró.");
				found = false;
			}
			j++;
        }	
        ...
        Request request = new Request(Uri.parse(mirrors[j]+archivo));
			...
			try {
				...
				Log.d("MIUIESUPDATER",sp.getString("directorio", "")+archivo+".md5sum");
				File md5sum = new File(sp.getString("directorio", "")+archivo+".md5sum");
				md5sum.createNewFile();
				String linea = br.readLine();
				Log.d("MIUIESUPDATER",linea);
				...
				if(dm.ACTION_DOWNLOAD_COMPLETE.equals(intent.getAction())){
					...
					if(c.moveToFirst()){
						...
						if(DownloadManager.STATUS_SUCCESSFUL == c.getInt(columnIndex)){
							...
							Toast.makeText(getApplicationContext(), "Ocurrió un error al descargar", Toast.LENGTH_LONG).show();
							i.putExtra("archivo", archivo);
