...
public class Principal extends Activity implements OnGestureListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(!android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)){
        	Toast not_sd = Toast.makeText(getBaseContext(), "No se pudo acceder a la tarjeta SD",Toast.LENGTH_LONG);
        	not_sd.show();
        	finish();
        }else{
	        ...
	        BroadcastReceiver br = new BroadcastReceiver() {
				...
				public void onReceive(Context context, Intent intent) {
					if(intent.getExtras().get("estado").equals("descargando")){
						check_update_info.setText("Descargando...");
    ...
    public void onResume(){
    	super.onResume();
    	if(!android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)){
        	Toast.makeText(myContext(), "No se pudo acceder a la tarjeta SD",Toast.LENGTH_LONG).show();
        	try {
				finalize();
			} catch (Throwable e) {
				// TODO Auto-generated catch block
	...
	private OnClickListener downloadUpdate = new OnClickListener() {
		...
		public void onClick(View v) {
			Intent i = new Intent();
			i.setClassName("miuies.updater", "miuies.updater.services.DownloadService");
	...
	private void readJson(){
		...
		if(!conexion){
			Toast sinConexion = Toast.makeText(Principal.this, "No hay conexion, por favor, active la conexión WiFi o de Datos.", 10);
			sinConexion.show();
		} else {
			final ProgressDialog buscando = ProgressDialog.show(Principal.this, "Buscando actualizaciones...", "Por favor, espere...", true);
			...
			final Handler handle = new Handler(){
		        ...
		        public void handleMessage(Message msg) {
		            ...
		            case 1:
						...
						break;
		            case 2:
		            	showToast("Error de conexión o problemas con el JSON.");
		            	buscando.dismiss();
		            	...
		            	break;
		            case 3:
		            	showToast("No se encontraron actualizaciones.");
		            	buscando.dismiss();
		            	romsOnline.setEnabled(false);
		            	romsOnline.setOnItemSelectedListener(null);
		            	romsOnline.setAdapter(new ArrayAdapter<String>(context, android.R.layout.simple_spinner_item, new String[0]));
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		if((velocityX > 400)&&(viewNum > 0)){
			Log.d("MIUIESUPDATER",velocityX + "  "+viewNum);
