...
public class OnlineJSON {
	...
	public OnlineJSON(){
		this.json = checkJSON();
		String[] versionStr = getModVersion("ro.modversion").split("\\.");
		int[] version = new int[versionStr.length];
		for(int i = 0; i < version.length; i++){
			version[i] = Integer.parseInt(versionStr[i]); 
		}
		this.miVersion = new Update("MIUIEs", "", "", version);
		buscarVersion();
	}
	...
	public JSONObject checkJSON(){
		try {
			...
			Looper.prepare();
			Principal.showToast("No se encontró el JSON, no se pueden comprobar las actualizaciones");
		} catch (JSONException e) {
			Looper.prepare();
			Principal.showToast("El JSON contiene errores, notifíquelo cuanto antes al cocinero");
		}
		return null;
	}
	public ArrayList<Update> buscarVersion(){
		...
		try {
			...
			for(int i = 0; i < incrementals.length(); i++){
				...
				if(versionIndicada.equals(miVersion.getVersion())){
